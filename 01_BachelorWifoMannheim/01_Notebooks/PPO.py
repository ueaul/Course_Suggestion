...
class PPOTrainer():
    ...
    def train_policy(self, obs, acts, old_log_probs, gaes):
        for _ in range(self.max_policy_train_iters):
            ...
            #calculate logits of new policy
            new_logits = self.ac.policy(obs)
            new_logits = Categorical(logits=new_logits)
            new_log_probs = new_logits.log_prob(acts)

            #create clipped ratio
...
def rollout(model, env, max_steps=1000):
    ...
    """
    ### Create data storage
    train_data = [[], [], [], [], []] # observation, act, reward, values, act_log_probs
    obs = env.reset()
...
for episode_idx in range(n_episodes):
  ...
  acts = torch.tensor(train_data[1][permute_idxs], dtype=torch.int32, device=DEVICE) #Action wäre hier eine Liste
